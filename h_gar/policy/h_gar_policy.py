...
from h_gar.model.common.normalizer import LinearNormalizer
...
from h_gar.model.autoregressive import mar_con_unified as mar
from h_gar.vae.vaekl import AutoencoderKL
from h_gar.utils.language_model import (
    get_text_model,
    extract_text_features,
)


class HGarPolicy(BaseImagePolicy):
    def __init__(
        self,
        vae_model_params,
    ...
    def load_pretrained_model(self):
        ...
        if "state_dicts" in pretrained_diffusion_model_ckpt:
            if "ema_model" in pretrained_diffusion_model_ckpt["state_dicts"]:
                ...
                self.copy_fine_net_from_net(self.model) 
            else:
    ...
    def copy_fine_net_from_net(self, model):
        ...
        net_state = model.diffloss.net.state_dict()
        fine_net_state = model.diffloss.fine_net.state_dict()

        copied_param_count = 0
        for key in fine_net_state.keys():
            # map key from fine_net → net
            net_key = key.replace("fine_net", "net")

            if any(key.endswith(suffix) for suffix in exclude_suffixes):
                continue

            if net_key in net_state:
                fine_net_state[key] = net_state[net_key].clone()
                copied_param_count += 1

        model.diffloss.fine_net.load_state_dict(fine_net_state, strict=False)
        print(f"Copied {copied_param_count} params from net to fine_net (excluding special layers).")
    def predict_action(
        ...
        B, T, C, H, W = obs_dict["image"].shape######4, 16, 3, 256, 256
        ...
        c, proprioception_input, _, x_goal = process_data(
    ...
    def compute_loss(self, batch, **kwargs):
        B, T, C, H, W = batch["obs"]["image"].size()#####2x32x3x256x256

        text_latents = None
        if self.language_emb_model == "clip":
            if "language" in batch["obs"]:
                language_goal = batch["obs"]["language"] ######2x32x2x30
                del batch["obs"]["language"]
                text_tokens = {
                    "input_ids": language_goal[:, 0].long()[:, 0], #####2x30
                    "attention_mask": language_goal[:, 0].long()[:, 1],######2x30
                }
                text_latents = extract_text_features(
                    self.text_model,
                    text_tokens,
        ...
        x, z, c, _, proprioception_input, latent_goal = get_vae_latent(
            x, self.vae_model, eval=False,  x_goal=x_goal, proprioception_input=proprioception_input
        )  #######z: (future_obs)2x4x16x16x16      c: (history_obs)2x4x16x16x16    latent_goal: （goal_image)2x1x16x16x16
