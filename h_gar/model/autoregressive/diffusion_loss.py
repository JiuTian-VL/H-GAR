...
class DiffLoss(nn.Module):
    ...
    def forward(self, target, z, mask=None, conf_score=None, text_latents=None, fine_grained = None):
        # different noise over t and s
        bsz, seq_len, _ = target.shape###2x1024x16
        target = target.reshape(bsz * seq_len, -1)
        z = z.reshape(bsz * seq_len, -1)####2x1024x768
        ...
        if fine_grained == 'True':
            loss_dict = self.train_diffusion.training_losses(
                self.fine_net, target, t, model_kwargs
            )
        else:
            loss_dict = self.train_diffusion.training_losses(
...
class SimpleMLPAdaLN(nn.Module):
    ...
    def forward(self, x, t, c):
        ...
        t = self.time_embed(t)
        c = self.cond_embed(c)
        # c = F.layer_norm(c, c.shape[-1:])  # or torch.tanh(c)
