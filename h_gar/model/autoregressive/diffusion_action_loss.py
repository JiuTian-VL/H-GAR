...
from h_gar.model.autoregressive.diffusion import create_diffusion
from h_gar.model.autoregressive.diffusion_loss import SimpleMLPAdaLN
...
class DiffActLoss(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.act_model_type == "conv_fc":
            self.w = 16
            self.h = 16
            self.spatial_token_len = self.w * self.h 
            self.num_frames = self.n_frames
            self.num_actions = 16
            ...
            self.interpolate_goal = nn.Linear(1, self.num_actions)
    ...
    def forward(self, target, z, task_mode=None, text_latents=None):
        ...
        if self.act_model_type == "conv_fc":
            # z = rearrange(z, "b (t s) c -> (b t) s c", t=self.n_frames)2x256x768
            B, L, C = z.shape
            T = L // self.spatial_token_len 
            assert L % self.spatial_token_len == 0, "z length must be divisible by 256"
            z = rearrange(z, "b (t s) c -> (b t) s c", t=T)####2x256x768->2x256x768

            z = rearrange(z, "b (w h) c -> b w h c", w=self.w)
            z = rearrange(z, "b w h c -> b c w h")#####2x768x16x16
            z = self.conv(z)
            z = rearrange(z, "b c w h -> b (c w h)")
            z = self.fc(z)

            z = rearrange(z, "(b t) c -> b t c", t=T)######2x4x768
            z = z.permute(0, 2, 1)
            if T == 1:
                # Use linear interpolation up to 16 steps
                z = self.interpolate_goal(z)
            elif T == 4:
                # Use trainable projection layer (predefined as nn.Linear(4, 16))
                z = self.interpolate(z)
            else:
                raise ValueError(f"Unsupported number of frames (T={T}) for action projection.")
            
            # z = self.interpolate(z)######2x768x16
            z = z.permute(0, 2, 1)
            z = self.refine(z)
