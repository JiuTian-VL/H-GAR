...
def select_frames(x, T, eval=False, select_timesteps=4, different_history_freq=False):
    if eval:
        ...
        if different_history_freq:####None
            indices = torch.cat(
                [
...
def process_data(batch, task_name="", eval=False, **kwargs):
    ...
    x = batch["obs"]["image"]#####2x32x3x256x256
    x = x * 255.0
    B, T, C, H, W = x.size()
    device = x.device
    x_goal = x[:, -1, :, :, :].unsqueeze(1)
    if "umi" in task_name:
        ...
        )######x:[2, 8, 3, 256, 256]      indices: [8]
    

    ## normalize image
    x = rearrange(x / 127.5 - 1, "b t c h w -> b c t h w")####2x3x8x256x256
    x_goal = rearrange(x_goal / 127.5 - 1, "b t c h w -> b c t h w")
    ...
    return x, proprioception_input, indices, x_goal
...
def get_vae_latent(x, vae_model, eval=False, x_goal=False, proprioception_input={}):
    train = not eval

    c, x = torch.chunk(x, 2, dim=2)###c: 2x3x4x256x256 x: 2x3x4x256x256  # take the first half as condition
    ...
    with torch.no_grad():
        ...
        latent_goal, latent_size = extract_latent_autoregressive(vae_model, x_goal)
    #####2x4x16x16x16   2x4x16x16x16
    return x, z, c, latent_size, proprioception_input, latent_goal
