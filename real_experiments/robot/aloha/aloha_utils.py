...
from real_experiments.robot.aloha.real_env import make_real_env
...
def get_next_task_label(task_label):
    ...
    if task_label == "":
        user_input = ""
        while user_input == "":
            user_input = input("No task input. Enter the task name: ")
        task_label = user_input
    else:
        user_input = input("Ready? leave blank to continue: ")
        if user_input == "":
